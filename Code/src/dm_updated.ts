...
const yesNoGrammar: {[index: string] : string} = {
yes: "yes",
yeah: "yes",
yep: "yes",
"of course": "yes",
...
function isInYesNoGrammar(utterance: string) {
  return utterance.toLowerCase() in yesNoGrammar;
}
...
function getAnswer(utterance: string) {
  return yesNoGrammar[utterance.toLowerCase()];
}
    ...
    AskFullDay: {
      ...
      on: {
        LISTEN_COMPLETE: [
          ...
          {
            guard: ({ context }) => !!context.lastResult && isInYesNoGrammar(context.lastResult![0].utterance) && getAnswer(context.lastResult![0].utterance) === "yes",
            target: "ConfirmFullDay",
          },
          {
            guard: ({ context }) => !!context.lastResult && isInYesNoGrammar(context.lastResult![0].utterance) && getAnswer(context.lastResult![0].utterance) === "no",
