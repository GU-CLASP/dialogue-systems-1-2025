...
const noGrammar: string[] = ["no", "no way", "nope", "that's wrong", "that's not what I said"]

const yesGrammar: string[] = ["yes", "yeah", "yep", "of course", "sure", "for sure", "that's what I said", "that's right"]
...
const dmMachine = setup({
  ...
  states: {
    ...
    Greeting: {
      ...
      on: {
        LISTEN_COMPLETE: [
          ...
          {
            target: ".MyBad",
            guard: ({ context }) => !!context.confidenceResult && context.confidence 
              && noGrammar.includes(context.confidenceResult![0]["utterance"].toLowerCase()),
            actions: assign({ confidence: false, confidenceResult: null })
          },
          {
            target: ".NotRelevant",
            guard: ({ context }) => !!context.confidenceResult && context.confidence 
              && yesGrammar.includes(context.confidenceResult![0]["utterance"].toLowerCase()),
            actions: assign({ confidence: false, confidenceResult: null })
          },
          {
            target: "Exit",
          ...
          {
            target: ".Prompt",
            guard: ({ context }) => !!context.lastResult && context.lastResult!["topIntent"] == "help",
          },
          {
            target: "Meeting",
          ...
          {
            target: "Done",
            guard: ({ context }) => context.reprompt == 3,
          },
          { target: ".NoInput" },
        ],
      },
      states: {
        ...
        Listen: {
          ...
          on: {
            RECOGNISED: [
              {
                guard: ({ context }) => context.confidence == true, 
                actions: assign(({ event }) => {
                  return { confidenceResult: event.value, reprompt: 0 }
                }),
              },
              {
                guard: ({ context }) => context.confidence == false,
                actions: assign(({ event }) => {
                  return { lastResult: event.nluValue, plainResult: event.value, reprompt: 0 }
                }),
              } 
            ],
        ...
        NoInput: {
          entry: { type: "spst.speak", params: { utterance: "I didn't hear you, please try again." } },
          on: { 
            SPEAK_COMPLETE: 
            [
              {
                target: "Listen",
                actions: assign(({ context }) => ({ reprompt: context.reprompt + 1 }))
              }
            ]
          },
        },
        NotRelevant: {
          entry: { type: "spst.speak", params: { utterance: "That's not relevant, please state your intent clearly." } },
          on: { SPEAK_COMPLETE: "Listen" },
        },
        Clarification: {
          entry: {
            type: "spst.speak",
            params: ({ context }) => ({
              utterance: `Did you say ${context.plainResult![0]["utterance"]} ?`,
            }),
          },
          on: { 
            SPEAK_COMPLETE: 
            [
              {
                target: "Listen",
                actions: assign({confidence: true})
              }
            ] 
          },
        },
        MyBad: {
            entry: { type: "spst.speak", params: { utterance: "Then I misunderstood you, please try again." } },
            on: { SPEAK_COMPLETE: "Listen" },
        },
      },
    },
    ...
    MeetingDay: {
      ...
      states: {
        Prompt: {
          entry: { type: "spst.speak", params: { utterance: "On what day is your meeting?" } },
          on: { SPEAK_COMPLETE: "Listen" },
        },
        Listen: {
          entry: { type: "spst.listen.nlu" },
    ...
    Done: {
      on: {
        CLICK: [
          {
            target: "Greeting",
            actions: assign({ meetingWith: null,  meetingDay: null, meetingTime: null, famousPerson: null, reprompt: 0, confidence: false }),
          } //reassign everything to avoid having old information when trying to book a new meeting
      ]},
    },
  },
});
