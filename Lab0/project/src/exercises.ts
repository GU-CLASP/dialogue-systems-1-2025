...
var guess = prompt("Guess a number between 1 and 10: ", "-1");
if (num == guess) {
    alert("Good work!");
} else {
...
// Q2 check if 50
function check50 (firstVar : number | string, secondVar: number | string){
...
// Q3 check paula
function checkIfName(query: string, names: string[]){
...
function findNameIndex(query: string, names: string[]){
...
// Q4 word creation
function makeMoreWord(word: string){
    if (word.length < 3){
...
function getNameLengths(names: string[]){
    var nameLengths = [];
    for (const name of names){
...
// Q6 zoos
interface listAnimals {
    [animal: string]: {"weight" : number, "origin": string} | Function;
}

let zooAnimals: listAnimals = {
...
function checkSpecificAnimal(animals: listAnimals, queryAnimal: string, queryType: "weight" | "origin", queryValue: string | number){
    if (queryAnimal in animals){
        if (typeof(animals[queryAnimal]) !== "function")
...
zooAnimals["about"] = function(name: string) {
